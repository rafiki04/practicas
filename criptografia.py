...
# Lee el archivo original byte por byte
...
# Función para convertir los bytes en ASCII
...
def generate_xor_files(byte_data):
    for i in range(256):  # Para cada valor de XOR desde 0x00 hasta 0xFF
        ...
        # Comprobueba si la palabra "This" está presente
        if contains_this(xor_result):
            # Convertierte el resultado XOR a ASCII
            ascii_result = to_ascii(xor_result)

            # Guarda el archivo con el nombre `xor_{i}.txt` (formato ASCII)
            filename = f'xor_{i:02X}.txt'
            with open(filename, 'w') as xor_file:
                xor_file.write(ascii_result)  # se escribe el resultado en formato ASCII
            print(f"Archivo ASCII generado: {filename}")

# Llama a la función para generar los archivos con XOR y verificar "This"
